...
describe('ThreadsService', () => {
    ...
    it('should show currentThreadMessages of Thread 2 all marked as read', () => {

        threadsService.setCurrentThread(t2);

        let result: any;
        threadsService.currentThreadMessages.subscribe(
            (response) => {
        ...
        chatMessagesService.addMessage(m1); // message on Thread 1
        chatMessagesService.addMessage(m2); // message on Thread 1
        chatMessagesService.addMessage(m3); // message on Thread 2
        chatMessagesService.addMessage(m4); // message on Thread 2
        chatMessagesService.addMessage(m5); // message on Thread 2
        chatMessagesService.addMessage(m6); // message on Thread 3

        expect(result.length).toBe(3);
        for (let i = 0; i < result.length; i++) {
            expect(result[i].isRead).toBe(true);
        }
